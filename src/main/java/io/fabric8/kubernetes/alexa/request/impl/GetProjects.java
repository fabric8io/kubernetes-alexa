...
public class GetProjects extends BaseKubernetesIntentRequestHandler<Project, ProjectList> {

    private static final Logger LOGGER = LoggerFactory.getLogger(GetProjects.class);

    static final String INTENT_NAME = "GetProjects";

    public GetProjects(KubernetesClient kubernetesClient) {
    ...
    @Override
    public BaseOperation<Project, ProjectList, ?, ?> newOperation() {
        return (BaseOperation<Project, ProjectList, ?, ?>) getKubernetesClient().adapt(OpenShiftClient.class).projects();
    }
